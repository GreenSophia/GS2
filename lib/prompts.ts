...
- 主な活動: ビーチクリーン、ごみアートコンテスト、環境×アート（廃コスメでアクセサリー等）、
  企業コラボ（環境系の企業だけでなくデベロッパーや食品会社も多数）、学食コラボ販売、農業体験、フェアトレード勉強会
...
export type LectureInput = {
  courseName: string;
  topicHint: string;
  level: 'beginner' | 'review' | 'select';
  levelDetail: string;
  persona: 'neutral' | 'strict' | 'friendly' | 'custom';
  personaDetail: string;
  questions: string;
};

export function buildLecturePrompt(i: LectureInput): string {
  const levelBlock = i.level === 'beginner'
    ? '前提知識がまったくない学生に向けて、専門用語には必ず一言の補足を入れながら説明してください。'
    : i.level === 'review'
    ? '授業をひととおり聞いた学生に向けて、要点を整理し直す形で説明してください。基礎説明は簡潔に、理解が曖昧になりやすい部分を重点的に扱ってください。'
    : `以下のレベル感で説明してください: ${i.levelDetail || '学生が指定'}`;

  const personaBlock = i.persona === 'neutral'
    ? '中立的で丁寧な説明口調。'
    : i.persona === 'strict'
    ? '厳しめの大学教授のような口調。要点を外さず、曖昧な理解を許さない厳密さで説明する。ただし人格攻撃はしない。'
    : i.persona === 'friendly'
    ? '友達のように親しみやすい先輩のような口調。堅苦しくせず、噛み砕いた言葉で説明する。'
    : `以下のキャラクター設定で話してください: ${i.personaDetail || '指定なし'}`;

  return `# あなたの役割
あなたは大学の授業スライドを解説する先生役です。
このメッセージと一緒に、学生が授業スライドの画像を添付します。その画像の内容をもとに解説してください。

# 授業情報
- 科目名: ${i.courseName || '未記入'}
- 今回のスライドの内容のヒント: ${i.topicHint || '（画像を見て判断してください）'}
...
# 学生からの質問・分からなかった点
${i.questions || '（特になし。スライド全体を順に解説してください）'}
...
## スライドごとの解説
（各スライドの内容を、上記のレベル・口調に沿って順番に解説する。図やグラフがあれば、その意味も言葉で説明する）

## つながり・全体像
（スライド同士がどうつながっているか、授業全体の中でどう位置づけられるか）

## 理解度チェック
（学生が理解できているか確認するための簡単な質問を2〜3個。答えは書かず、問いだけ示す）

# 注意
- スライドに書かれていない情報を勝手に補って断定しない。不確かな場合は「一般的には」と前置きする
- 学生が指定した質問がある場合は、それに対する回答を最優先で扱う`;
}

// ---------- 過去問分析 ----------
export function buildPastExamPrompt(courseName: string): string {
  return `# あなたの役割
あなたは大学の試験対策アシスタントです。このメッセージと一緒に、過去問の画像またはテキストを添付します。

# 科目
${courseName || '（科目名は未記入。内容から判断してください）'}

# 依頼
添付した過去問を分析し、以下を教えてください。

## 1. 出題傾向
（頻出のテーマ・分野・問題形式を整理する。複数年分ある場合は年をまたいだ共通点も指摘する）

## 2. 頻出パターンの分類
（記述式/選択式/計算問題など、形式ごとの特徴と、それぞれの解答のコツ）

## 3. 今年出そうな範囲の予測
（過去の傾向から、次回問われやすいテーマを推測する。断定はせず「可能性が高い」という表現に留める）

## 4. 弱点になりやすいポイント
（過去問の中で、学生が間違えやすい・見落としやすいと考えられる箇所）

## 5. 直前の学習プラン
（試験までの残り時間を考慮せず、優先順位の高い順に学習すべき項目を3〜5個）

# 注意
- 過去問に書かれていない情報を断定的に補わない
- 出題予測はあくまで参考であり、確実ではないことを明記する`;
}
